...
Классы и функции, предоставляемые Boost.IOStreams, определены в пространстве имен `boost::iostreams`. Здесь нет основного заголовочного файла, а так как Boost.IOStreams содержит не только заголовочные файлы, она должна быть заранее собрана. Это важно, потому что поддержка некоторых функций может отсутствовать в зависимости от того, как Boost.IOStreams будет скомпилирована.

<a name="devices"></a>
##Устройства

Устройства - это классы, которые обеспечивают доступ для чтения и записи в объекты, которые обычно не участвуют в выполнении программы, например файлы. Однако вы можете получить доступ к внутренним объектам, например массивам, как к устройствам.
...
[Пример 34.6](#Ex346) применяет `boost::iostreams::array_sink`, чтобы записать данные в массив. Данные проходят через фильтр `boost::iostreams::regex_filter`, заменяющий символы. Фильтр принимает регулярное выражение и строку форматирования. Регулярное выражение описывает то, что надо заменить. Строка форматирования определяет, чем должны быть заменены символы. Пример заменяет "Boost" на "C++". Фильтр будет соответствовать одному или нескольким последовательным экземплярам буквы "о" в "Boost". 
...
Фильтры должны быть переданы перед устройствами, порядок их передачи важен. Вы можете передавать один или несколько фильтров, но в тот момент, когда устройство передано, поток завершится, и после этого вызывать `push()` нельзя.
...
Boost.IOStreams поддерживает несколько видов фильтров, касающихся сжатых данных. `boost::iostreams::zlib_compressor` сжимает данные в формате ZLIB. Чтобы разархивировать данные из этого формата, используют `boost::iostreams::zlib_decompressor`. Фильтры добавляются в поток при помощи `push()`. 
...
Библиотека [Boost.Filesystem](http://www.boost.org/doc/libs/1_62_0/libs/filesystem/doc/index.htm) позволяет легко работать с файлами и каталогами. Она предоставляет класс, который называется **`boost::filesystem::path`**, он работает с путями. Кроме того, многие автономные функции способны создавать директории или проверять существование файла. 
...
`boost::filesystem::path` может быть инициализирован с помощью многобайтовых строк. Многобайтовые строки интерпретируются как строки Unicode, что позволяет создавать пути, используя символы почти любого языка. Это очень важное отличие от Boost.Filesystem 2, которая предоставляла различные классы, такие как `boost::filesystem::path` и `boost::filesystem::wpath`, для различных типов строк. 
...
Метод `make_preferred()` ничего не делает на POSIX-совместимых операционных системах, таких как ОС Linux.
...
`boost::system::system_error` и `boost::system::error_code` описаны в [Главе 55](https://theboostcpplibraries.com/boost.system). В дополнение к наследуемому интерфейсу от `boost::system::system_error`, `boost::filesystem::filesystem_error` предоставляет двa метода `path1()` и `path2()`, каждый из которых возвращает объект типа `boost::filesystem::path`. Поскольку существуют функции, которые принимают два параметра типа `boost::filesystem::path`, эти два метода обеспечивают простой способ получения переданных путей в случае ошибки.
...
`boost::filesystem::space()` получает общее и свободное дисковое пространство (см. [Пример 35.13](#Ex3513)). Он возвращает объект типа `boost::filesystem::space_info`, который предоставляет три открытых поля: **capacity**, **free**, и **available** типа `boost::uintmax_t`, измерающиеся в байтах.
...
<blockquote>В примере 35.14 boost::filesystem::remove() вызван явно, используя пространство имен. Иначе Visual C++ 2013 перепутал бы remove() с функцией из заголовочного файла ***stdio.h***.</blockquote>
...
В [примере 35.15](#Ex3515) представлена функция, которая создает абсолютный путь на основе имени файла или части пути. Выведенный на экран путь зависит от каталога, из которого запущена программа. Например, если бы программа была запущена в ***C:\***, вывод был бы **`"C:\photo.jpg"`**.
...
`boost::filesystem::directory_iterator` инициализируется с помощью пути, для получения итератора, указывающего на начало каталога. Чтобы извлечь конец каталога, класс должен быть создан с помощью конструктора по умолчанию. Записи могут быть созданы или удалены во время цикла без его нарушения. Тем не менее, не определено, будут ли изменения видимыми во время цикла или нет. Например, итератор не может указывать на только что созданные файлы. Для того, чтобы убедиться, что все текущие записи доступны, нужно перезапустить цикл. 
