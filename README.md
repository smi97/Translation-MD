...
Возвращаемые значения методов, возвращающих нативные пути, зависят от операционной системы, на которой выполняется программа, а методов, возвращающих универсальные пути, нет. Универсальные пути уникальным образом идентифицируют файлы и каталоги, независимо от операцонной системы и, таким образом, позволяют легко писать платформенно-независимый код.
...
Конструктор `boost::filesystem::path`  поддерживает как универсальные, так и платформенно-зависимые пути. В [Примере 35.3](#Ex353) путь "C:\\Windows\\System" является определенным ОС Windows и не может быть перенесен на другие ОС. Заметьте также, что, потому как обратный слэш в C++ - escape-последовательность, перед ним нужно поставить еще один обратный слэш. Этот путь будет корректно распознан Boost.Filesystem только если программа запущена на ОС Windows. При ее выполнении на POSIX системе, например Linux, этот пример вернет **`C:\Windows\System`** для всех методов. Так как обратный слэш не используется в качестве разделителя на Linux как в портируемом, так и в собственном формате, Boost.Filesystem не распознает этот символ как разделитель для файлов и каталогов.

Макро BOOST_WINDOWS_API, предоставленный Boost.System, определен, если пример скомпилирован на Windows. Соответствующим макросом для операционных систем POSIX является BOOST_POSIX_API.
...
Есть еще два метода, которые могут разделить имя файла на его компоненты. Их можно вызвать только, если has_filename() возвращает true. Иначе они возвращают пустые строки, так как при отсутствии имени файла нечего разделять. 
...
Вместо того, чтобы получать доступ к компонентам пути через вызовы методов, Вы можете также проходить по компонентам итерационно.
...
Помимо оператора `operator/=`, Boost.Filesystem обеспечивает методы `remove_filename()`, `replace_extension()`, `make_absolute()`, и `make_preferred()` для изменения путей. Последний упомянутый метод в частности разработан для использования на ОС Windows.
...
Обратите внимание на то, что метод `make_preferred()` не только возвращает переделанный путь, но также и изменяет объект, у которого он был вызван. **p** содержит "C:\Windows\System" после вызова.
...
До представления различных функций важно понять то, что происходит в случае ошибки. Все методы вызывают функции операционной системы, которые могут перестать работать. Поэтому Boost.Filesystem обеспечивает два варианта функций, которые ведут себя по-разному в случае ошибки:

- Первый вариант бросает исключение типа `boost::filesystem::filesystem_error`. Этот класс является производным от `boost::system::system_error` и таким образом вписывается в платформу Boost.System. 
...
[Пример 35.11](#Ex3511) использует объект типа `boost::system::error_code`, который необходимо оценить в явном виде, чтобы определить, был ли успешным вызов boost::filesystem::file_size().
...
Чтобы определить время, когда файл был изменен в последний раз, можно использовать `boost::filesystem::last_write_time()` (см. [Пример 35.12](#Ex3512)).
...
В то время как функции, представленные выше не затрагивают файлы и каталоги, существует несколько функций, которые могут быть использованы для создания, переименования или удаления файлов и каталогов.
...
[Пример 35.17](#Ex3517) вызывает `boost::filesystem::current_path()` много раз. Если функция вызывается без параметров, текущий рабочий каталог возвращается. Если передается объект типа `boost::filesystem::path` текущий рабочий каталог устанавливается.
...
`boost::filesystem::directory_iterator` инициализируется с помощью пути, чтобы получить итератор, указывающий на начало каталога. Чтобы извлечь конец директории, класс должен быть создан с помощью конструктора по умолчанию. Записи могут быть созданы или удалены при итерации без ее нарушения. Тем не менее, не определено будут ли изменения видимыми во время итерации. Например, итератор не может указывать на только что созданные файлы. Для того, чтобы убедиться, что все текущие записи доступны, нужно перезапустить итерацию. 

Чтобы рекурсивно перебирать каталоги и подкаталоги, Boost.Filesystem предоставляет итератор `boost::filesystem::recursive_directory_iterator`. 
