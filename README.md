...
Стоит заметить, что в `boost::iostreams::array_source` и `boost::iostreams::array_sink` передается ссылка на массив. Массив не должен быть разрушен, когда устройство еще используется.Массив должен быть работающим, пока используется устройство.
...
В [примере 34.3](#Ex343) применяется устройство типа `boost::iostreams::back_insert_device` вместо `boost::iostreams::array_sink`. Оно может применяется при записи данных в любой контейнер, поддерживающий метод `insert()`. Этот метод здесь необходим для записи данных в контейнер.

В примере используется `boost::iostreams::back_insert_device`, чтобы записать в вектор строки "Boost". Затем эта строка читается из `boost::iostreams::array_source`. Ссылка на начало вектора и его размер передаются в качестве параметров конструктора `boost::iostreams::array_source`.
...
Для записи данных в файлы Boost.IOStreams применяет метод `boost::iostreams::file_sink` и `boost::iostreams::mapped_file_sink`.
...
Этот класс использует функцию `push()`, в которую передаются фильтры и устройства.
...
Библиотека [Boost.Filesystem](http://www.boost.org/doc/libs/1_62_0/libs/filesystem/doc/index.htm) позволяет легко работать с файлами и каталогами. Она использует класс, которые называется **`boost::filesystem::path`**, он обрабатывет пути. Кроме того, многие автономные функции способны создавать директорит или проверять существование файла. 
...
Значения методов, возвращающих нативные пути, зависят от операционной системы, на которой выполняется программа. Методов, возвращающих универсальные пути, нет. Универсальные пути уникальным образом идентифицируют файлы и каталоги, независимо от операцонной системы и, таким образом, позволяют легко писать платформенно-независимый код.
...
Конструктор `boost::filesystem::path`  поддерживает как универсальные, так и платформенно-зависимые пути. В [Примере 35.3](#Ex353) путь "C:\\Windows\\System" является определенным на ОС Windows и не может быть перенесен на другие ОС. Потому как обратный слэш в C++ - это escape-последовательность, стоит заметить, что перед ним нужно поставить еще один обратный слэш. Этот путь будет корректно распознан Boost.Filesystem, если программа запущена на ОС Windows. При ее выполнении на POSIX системе, например Linux, этот пример вернет **`C:\Windows\System`** для всех методов. Так как обратный слэш не используется в качестве разделителя на Linux как в портируемом, так и в собственном формате, Boost.Filesystem не распознает этот символ как разделитель для файлов и каталогов.
...
Существует еще два метода, которые могут разделить имя файла на его компоненты. Их можно вызвать, если has_filename() возвращает true. Иначе они возвращают пустые строки, так как при отсутствии имени файла нечего разделять. 
...
В [примере 35.15](#Ex3515) представлена функция, которая создает абсолютный путь на основе имени файла или части пути. Выведенный на экран путь зависит от того, из какого каталога запущена программа. Например, если бы программа была запущена в ***C:\***, вывод был бы **`"C:\photo.jpg"`**.
...
[Пример 35.17](#Ex3517) вызывает `boost::filesystem::current_path()` несколько раз. Если функция вызывается без параметров, возвращается текущий рабочий каталог. Если же передается объект типа `boost::filesystem::path`, то текущий рабочий каталог устанавливается.
...
[Пример 35.19](#Ex3519) открывает файл при помощи класса `boost::filesystem::ofstream`. Объект типа `boost::filesystem::path` передается в конструтор `boost::filesystem::ofstream`. Метод `open()`  принимает параметр типа `boost::filesystem::path`.
